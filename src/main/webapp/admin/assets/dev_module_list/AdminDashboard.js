...
$(document).ready(function() {
	...
	$("#hideoninstitute").show(); // show institute dropdown by default
...
function Searchchart(year) {
	...
	$
		.ajax(
			{
				url: '../admin/getallinstituteblogchart',
				type: "POST",
//				data: { "year": year },
				cors: true,
				dataType: 'json',
		...
		.done(
			function(data) {
				...
				if (data.status == '1') {
					...
					CreateChart3('chartdiv3', data.data);
...
function CreateChart3(divname, data) {
	...
	am5.ready(function() {
		...
		xRenderer.labels.template.setAll({
		    rotation: -45,  // Rotate labels at -45 degrees
		    textAlign: "center",
			fontSize:'14px',
		});
		...
		var filteredData = data.filter(item => item.institute  > 0)
...
//		makeSeries("female", "female");
		...
		// Add hover behavior for yAxis
		yAxis.events.on("pointermove", function(ev) {
			const position = yAxis.toGlobal(ev.point);
			const axisValue = yAxis.positionToValue(position.y);

			// Find the closest data point
			const closestData = data.reduce((prev, curr) => {
				return Math.abs(curr.total - axisValue) < Math.abs(prev.total - axisValue) ? curr : prev;
			});

			// Show tooltip with "male" and "female" values
			yAxis.set("tooltipText", `Male: ${closestData.male}, Female: ${closestData.female}`);
		});
