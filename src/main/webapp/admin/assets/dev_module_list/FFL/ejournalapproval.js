...
function dataTable(tableName) {
	var table = $('#' + tableName).DataTable({
		...
		"columnDefs": [
			{
				"targets": [0, 5, 11, 12], // The column index you want to disable sorting for (0-based)
				"orderable": false,
			}]
...
function data(tableName) {
	...
	$.ajax(
		{
			url: '../admin/LoadApprovalJournalData?pageno=' + currentPage + '&&length=' + pageLength + "&&search=" + Search + "&&order=" + order,
			type: "POST",
...
function RejectApproval(id, remarks) {
...
function setevents() {
	...
	document.querySelectorAll('.approvecls1').forEach((items, index) => {
		items.addEventListener('click', event => {
			...
			$.confirm({
				title: '',
				content: 'Are You Sure You Want to Accept this Journal ?<br><br>',
