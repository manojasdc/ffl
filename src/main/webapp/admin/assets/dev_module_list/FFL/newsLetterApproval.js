$(document).ready(function() {
	...
	mockjax1('newsLettertbl');
	dt = dataTable('newsLettertbl');
...
function data(tableName) {
	...
	$.ajax(
		{
			url: '../admin/LoadNewsLetterApprovalData?pageno=' + currentPage + '&&length=' + pageLength + "&&search=" + Search + "&&order=" + order,
			type: "POST",
...
function setevents() {
	...
	document.querySelectorAll('.approvecls1').forEach((items, index) => {
		items.addEventListener('click', event => {
			...
			$.confirm({
				title: '',
				content: 'Are You Sure You Want to Approve this News Letter ?<br><br>',
	...
	document.querySelectorAll('.rejectcls').forEach((items, index) => {
		items.addEventListener('click', event => {
			...
			$.confirm({
				title: '',
				content: 'Are You Sure You Want to Reject this NewsLetter?<br><br>' +
...
function rejectLetter(id, remarks) {
	$('#work').block({ message: 'Please wait....' });
	var jsondata = {
		"id": id,
