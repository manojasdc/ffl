...
function dataTable(tableName) {
	var table = $('#' + tableName).DataTable({
		...
		"columnDefs": [
			{
				"targets": [0, 4, 5], // The column index you want to disable sorting for (0-based)
				"orderable": false,
			}],
...
function data(tableName) {
	...
	$.ajax(
		{
			url: '/FriendsForLife/admin/LoadActivityApprovalData?pageno=' + currentPage + '&&length=' + pageLength + "&&search=" + Search + "&&order=" + order
				+ "&&category=" + category,
			type: "POST",
...
function Getimageforactivity(id) {
	...
	$
		.ajax(
			{
				url: '/FriendsForLife/admin/Getactimageapproval',
				type: "POST",
				data: JSON
					.stringify(jsondata),
				contentType: 'application/json',
...
function RejectApprovalforactivity(id, remarks) {
	...
	$
		.ajax(
			{
				url: '../admin/RejectApprovalforactivity',
				type: "POST",
				data: JSON
					.stringify(jsondata),
				contentType: 'application/json',
					...
					$.confirm({
						...
						buttons: {
							...
							OK: {
								...
								keys: ['enter', 'shift'],
								action: function() {
									var table = $('#activitytbl').DataTable();
									table.ajax.reload();
								}
							}
						}
...
function Acceptapprovalforactivity(id) {
	$('#work').block({ message: 'Please wait....' });
	...
	$
		.ajax(
			{
				url: '../admin/Acceptapprovalforactivity',
				type: "POST",
				data: JSON
					.stringify(jsondata),
				contentType: 'application/json',
						...
						OK: {
							...
							action: function() {
								...
								if (remarks.trim() === "") {
									...
									RejectApprovalforactivity(hid, remarks);
								}
							}
						}
					},
