...
function dataTable(tableName) {
	var table = $('#' + tableName).DataTable({
		...
		"columnDefs": [
			{
				"targets": [0,7,8], // The column index you want to disable sorting for (0-based)
				"orderable": false,
			}],
...
function data(tableName) {
	...
	$.ajax(
		{
			url: '../admin/LoadAlumniApprovaldata?pageno=' + currentPage + '&&length=' + pageLength + "&&search=" + Search+"&&order=" + order,
			type: "POST",
...
function AlumniRejectApproval(id, remarks) {
	...
	$
		.ajax(
			{
				url: '../admin/AlumniRejectApproval',
				type: "POST",
				data: JSON
					.stringify(jsondata),
				contentType: 'application/json',
...
function AlumniAcceptapproval(id) {
	$('#work').block({ message: 'Please wait....' });
	...
	$
		.ajax(
			{
				url: '../admin/AlumniAcceptapproval',
				type: "POST",
				data: JSON
					.stringify(jsondata),
				contentType: 'application/json',
						...
						OK: {
							...
							action: function() {
								...
								if (remarks.trim() === "") {
									...
									AlumniRejectApproval(hid, remarks);
								}
							}
						}
					},
		...
		.done(
			function(data) {
				if (data.data.DocumentImageType == "jpeg" || data.data.DocumentImageType == "jpg" || data.data.DocumentImageType == "png") {
					...
					document.getElementById("idalumni1").value = data.data.mapid;
